...
> 状态:**FROZEN**(v1.0,冻结于 **2026-09-13**)。
> 冻结后任何设计性修改须走变更流程(决策清单 B 区新增项 + 附失效场景,先例见 §10);**运行值**(部署地址、按"截至执行日"定义的轮数)在部署时填入即可,不构成对本文件的修改(见 §8 注)。
...
- 🔴 **"返回零 ⇒ 该块未提交"靠什么成立(2026-09-14 补,B12 / 提交侧守卫)** [通用]
  ...
  ① canon 信号(可靠,推荐读取端用它)
     ...
     ⇒ canon == 0 ⟺ 从未提交
     ✅ **这是合约不变量**(2026-09-14 加固):构造函数 `revert InvalidCanon(0)`,
        故 CANON 不可能为 0。此前它只是"部署侧保证"——活在一台机器上的部署脚本里,
        而第三方在 Blockscout 上看到的是 `CANON = canon_` 加一句注释:
        以 `canon_ = 0` 部署会成功、提交会成功、测试会全绿,而【接口自己声明的那条保证对那个部署是假的】。
        **保证必须和它被声明的地方在一起。** 边界取 `== 0` 不取 `< 2`:0 是缺席信号占的那个值,
        是这个不变量真正需要的最小边界;`< 2` 会把 §7 的版本表焊进合约。
  ② 哈希信号
  ...
  ⇒ 判据:D4 读取端**可以依赖 canon == 0**(合约不变量);若依赖零哈希,就必须同时引用提交侧这条保证。
  ```
...
- 🔴 **单调水印的失败场景与逃生口(2026-09-14 补,决策 B11;部署【之前】写入)** [通用]

  失效路径走一遍:
  ```
  误提交一条 block = 99,999,999(手打错、脚本读错字段、或 owner 热钥泄露)
    ⇒ 水印 = 99,999,999,而数据源链现在才 6,240 万
    ⇒ 此后每一批合法提交都低于水印 ⇒ 永久拒绝
    ⇒ 合约没有逃生口,也不该有 —— 那正是 append-only 的意思
  ```
  **逃生口不在合约里,在锚的多重性:账本不是唯一锚,git 历史才是。**
  毒死一个账本,代价是【**重新部署 + 重新公布地址**】,**不是证据丢失** ——
  数据、逐轮哈希与 git 历史都还在,错误档案照常公开,而 `deployments.jsonl` 让"换过地址"这件事本身可查。
  ⇒ 本条把最坏后果限定为"换一个地址",而不是"失去可验证性"。
  ⇒ **这一段必须在部署之前写入**:事后第一次说出口,它听起来就是借口(先例:§5 的边界补记,2026-09-12)。

  三层**链下**守卫,不动合约、不动冻结接口:
  ```
  ① 提交侧:拒绝任何 block > 数据源链(4663)当前链头
     ⚠️ 是【数据源链】不是账本所在链:46630 链头 ≈119M ≫ 轮次 54–62M
        ⇒ 对着账本链的头,守卫永不触发 —— 这正是 B8 否掉链上版本的同一条理由
     ✅ 工具:tools/round_count.py --source-rpc(缺 RPC 时明说"SKIPPED",不静默放过)
  ② 部署脚本:拒绝在非预期 chainId 上 broadcast(必须显式声明 LEDGER_CHAIN_ID)
  ③ owner 热钥纪律 + D9 看门狗(非我方提交即告警)
  ```
  ⇒ 判据与第 10 条同源:**约束放在失败发生的地方,不放在合约里。**
