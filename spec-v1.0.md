...
- C6(demo 依赖绑定,2026-09-09 落):演示须含**失败路径**——篡改一个字节 → Verify 当场红(与 C4 链上验证同一步;零代码增量,verify.py 现成)。🔴 **"未篡改→绿"依赖链上真有承诺**:截至 **09-11** 账本为空(**353 轮已发布,`committed` 0/353**)——绿镜头依赖 D6 主网回填(9/25)或**测试网 46630 已提交轮次(9/21 起,plan B)**。**C6 继承 D6 风险,与素材清单"早轮真实轮次 verify 镜头"绑在一起排期,不单独排。**
